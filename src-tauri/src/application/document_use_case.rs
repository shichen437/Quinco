...
impl<R: DocumentRepository> DocumentUseCase<R> {
    ...
    pub async fn get_workspace_documents(&self, wid: i64) -> Result<Vec<Document>, DomainError> {
    ...
    pub async fn get_deleted(&self, wid: i64) -> Result<Vec<Document>, DomainError> {
        self.repo.get_deleted(wid).await
    }
