...
import type { GraphData } from "@/api/tauri-bridge/graph"
...
function getTheme(isDark: boolean) {
  return {
    ...
    deleted: isDark ? "#525252" : "#a1a1aa",
    edge: isDark ? "rgba(148, 163, 184, 0.85)" : "rgba(100, 116, 139, 0.6)",
    edgeHighlight: isDark ? "#c084fc" : "#7c3aed",
    label: isDark ? "#a1a1aa" : "#52525b",
    labelHighlight: isDark ? "#fafafa" : "#09090b",
    bg: isDark ? "oklch(0.145 0 0)" : "#ffffff",
    glowColor: isDark ? "#a855f7" : "#7c3aed",
    glowDeleted: isDark ? "#525252" : "#a1a1aa",
    fadeOpacity: 0.08,
  }
}
...
export default function GraphCanvas({ data }: GraphCanvasProps) {
  ...
  const openTab = useTabStore((s) => s.openDocInActiveTab)

  const [isDark, setIsDark] = useState(() => document.documentElement.classList.contains("dark"))
  const [hoveredNode, setHoveredNode] = useState<string | null>(null)
  const [isReady, setIsReady] = useState(false)
  const [stats, setStats] = useState({ nodes: data.nodes.length, edges: data.links.length })

  const adjacencyRef = useRef<Map<string, Set<string>>>(new Map())
  const nodeDegreeRef = useRef<Map<string, number>>(new Map())
  ...
  useEffect(() => {
    ...
    const degrees = nodeDegreeRef.current
    ...
    data.nodes.forEach((node, i) => {
      ...
      const degreeScale = 1 + Math.min(degree / maxDegree, 1) * 0.6
      const nodeSize = sizes.nodeRadius * degreeScale
      graph.addNode(nodeId, {
        x: pos.x,
        y: pos.y,
        size: nodeSize,
        color: node.is_delete ? theme.deleted : theme.primary,
        docId: node.id,
        isDelete: node.is_delete,
        label: truncate(node.title || tCommon("unnamed"), 12),
        emoji: node.emoji,
        finalX: pos.x,
        finalY: pos.y,
    ...
    const validEdgeCount = data.links.filter((edge) => {
      const s = docIdToNodeId.get(edge.source)
      const t = docIdToNodeId.get(edge.target)
    ...
    const sigma = new Sigma(graph, container, {
      ...
      edgeReducer(_edge, d) {
        return { ...d, color: theme.edge, zIndex: 1 }
      },
      nodeReducer(_node, d) {
        return { ...d, zIndex: 0 }
      },
    ...
    data.nodes.forEach((node, i) => {
      const nodeId = `n${i}`
      const nodeSize = graph.getNodeAttribute(nodeId, "nodeSize") as number
      const glowSize =
        sizes.glowSize *
        (0.8 + (((graph.getNodeAttribute(nodeId, "degreeScale") as number) - 1) / 0.6) * 0.4)
      const wrapper = document.createElement("div")
      wrapper.dataset.nodeId = nodeId
      wrapper.dataset.isDelete = String(node.is_delete)
      wrapper.dataset.docId = node.id
      ...
      glow.style.cssText = `
        ...
        background:radial-gradient(circle, ${node.is_delete ? theme.glowDeleted : theme.glowColor} 0%, transparent 70%);
        filter:blur(3px);
        opacity:0.7;
        pointer-events:none;
        z-index:1;
      ...
      circle.style.cssText = `
        position:absolute;
        width:${nodeSize * 2}px;height:${nodeSize * 2}px;
        border-radius:50%;
        transform:translate(-50%,-50%) scale(0.3);
        background:${node.is_delete ? theme.deleted : theme.primary};
        box-shadow: 0 0 ${nodeSize}px ${node.is_delete ? "transparent" : theme.primary};
      ...
      label.textContent = `${emoji}${truncate(node.title || tCommon("unnamed"), 12)}`
    ...
    function animateProgressive(now: number) {
      ...
      nodeEls.forEach((el, index) => {
        const staggerDelay = (index / nodeEls.length) * 0.3
        ...
        if (nodeT > 0) {
          ...
          if (label) {
            const isDel = el.dataset.isDelete === "1"
            label.style.opacity = String(nodeT * (isDel ? 0.4 : 0.9))
          }
      ...
      if (t < 1) {
        ...
        nodeEls.forEach((el) => {
          ...
          const circle = el.querySelector<HTMLElement>(".g-node-circle")
          const label = el.querySelector<HTMLElement>(".g-node-label")
          if (circle) circle.style.transform = "translate(-50%,-50%) scale(1)"
          if (label) {
            const isDel = el.dataset.isDelete === "1"
            label.style.opacity = isDel ? "0.4" : "0.9"
          }
        })
    ...
    sigma.on("clickNode", ({ node }) => {
      ...
      if (docId && !isDelete) openTab(docId, title || tCommon("unnamed"))
    })
  ...
  useEffect(() => {
    ...
    if (hoveredNode) {
      const docId = graph.getNodeAttribute(hoveredNode, "docId") as string
      const neighbors = adj.get(docId) ?? new Set()

      graph.forEachNode((node) => {
        const nodeDocId = graph.getNodeAttribute(node, "docId") as string
        const isActive = node === hoveredNode || neighbors.has(nodeDocId)
        ...
        if (circle) {
          if (node === hoveredNode) {
            ...
            circle.style.boxShadow = `0 0 24px ${theme.primary}`
          } else {
            circle.style.transform = "translate(-50%,-50%) scale(1)"
            circle.style.boxShadow = isActive ? `0 0 10px ${theme.primary}` : "none"
          }
        }
        ...
        if (label) {
          ...
          label.style.fontWeight = node === hoveredNode ? "600" : "400"
        }
      })
      ...
      graph.forEachNode((node) => {
        const isDel = graph.getNodeAttribute(node, "isDelete") as number
        graph.setNodeAttribute(node, "color", isDel ? theme.deleted : theme.primary)
        ...
        if (circle) {
          circle.style.transform = "translate(-50%,-50%) scale(1)"
          circle.style.boxShadow = isDel ? "none" : `0 0 ${sizes.nodeRadius}px ${theme.primary}`
        }
        if (label) {
          label.style.opacity = isDel ? "0.4" : "1"
  ...
  }, [hoveredNode, isDark, data.nodes.length, isReady, data])
  ...
  useEffect(() => {
    ...
    graph.forEachNode((node, attrs) => {
      const isDel = attrs.isDelete as number
      graph.setNodeAttribute(node, "color", isDel ? theme.deleted : theme.primary)
      ...
      if (glow) {
        glow.style.background = `radial-gradient(circle, ${
          isDel ? theme.glowDeleted : theme.glowColor
        } 0%, transparent 70%)`
      }
      if (circle) {
        circle.style.background = isDel ? theme.deleted : theme.primary
        circle.style.boxShadow = `0 0 ${sizes.nodeRadius}px ${
          isDel ? "transparent" : theme.primary
        }`
      }
      if (label) {
        label.style.color = theme.label
        label.style.textShadow = `0 1px 2px ${isDark ? "rgba(0,0,0,0.8)" : "rgba(255,255,255,0.9)"}`
      }
    })
  ...
  return (
    <div className="relative flex h-full w-full flex-col">
      ...
      <div
        ...
        style={{ color: getTheme(isDark).label, opacity: 0.7 }}
      >
        <span>{t("nodes", { count: stats.nodes })}</span>
        <span>·</span>
        <span>{t("edges", { count: stats.edges })}</span>
      </div>
    </div>
  )
}
